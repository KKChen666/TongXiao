...
### 12.1 部署方式
项目必须使用 **Docker 容器** 方式部署，不允许直接裸机运行。

- 使用项目根目录的 `Dockerfile`（多阶段构建：Node 22 构建前端 + Python 3.10-slim 运行后端）
- 使用 `docker compose up -d --build` 构建并启动
- 容器名：`tongxiao`
- 端口映射：`7896:7896`
- 重启策略：`unless-stopped`
...
# 查看日志
docker logs -f tongxiao

# 重启容器
docker restart tongxiao
```
